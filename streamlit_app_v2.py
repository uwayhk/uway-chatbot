...
def format_knowledge_for_prompt(knowledge: dict) -> str:
    """Format knowledge base content for system prompt"""
    if not knowledge:
        return ""
    
    formatted = "\n\n=== KNOWLEDGE BASE ===\n"
    formatted += "You have access to the following proprietary knowledge base. Use this information to answer questions accurately:\n\n"
    ...
    formatted += "2. For questions about Uway Innovation Limited, refer to the company profile\n"
    formatted += "3. For KYC/AML best practices, refer to Sumsub guidelines\n"
    formatted += "4. Always be transparent and accurate - if information is not in your knowledge base, acknowledge this\n"
...
class OpenRouterProvider:
    """Fallback 2: Qwen via Hugging Face Inference API"""
    def __init__(self):
        self.hf_token = os.environ.get("HF_TOKEN")
        self.model = "Qwen/Qwen3.5-35B-A3B"  # Qwen 3.5 35B model on HF
    ...
    def chat(self, messages: list, system_prompt: str, stream: bool = True):
        ...
        # Use chat_completion() API (newer huggingface_hub versions)
...
IMPORTANT: This is for informational purposes only. Always recommend consulting licensed compliance professionals for critical business decisions."""
    ...
    kb_content = format_knowledge_for_prompt(knowledge_base)
...
with st.sidebar:
    ...
    st.markdown("2️⃣ Gemini 2.5 (Fallback)")
    st.markdown("3️⃣ Qwen 3.5 via HF (Fallback)")
    ...
    st.divider()
    st.info("💡 Powered by NVIDIA NIM + Qwen 3.5\n\n📚 Knowledge: Sumsub KYC + Uway Profile")
